...
public class C1_JavaMathClassMethods {
    public static void main(String[] args) {
        ...
        // random () methoduyla rastgele değer uretme
        double dbSayi= Math.random()*100;//
        System.out.println("dbSayi = " + dbSayi);

        // int değer üretme
        int intSayi= (int) (Math.random()*50);// random double sayı üretir type castingle int yaptık 0 ile 50 arası sayı
        ...
        //javaMath.Random Class buraya kadar Math.Random clası kullandık

        for (int i = 0; i < 10; i++) {
            System.out.println(i+1+" . sayi -->  "+ Math.random());// 10 sayı ürettik 0-1 arası


        }
        //Random Classı

        Random random= new Random();//random diye bir obje oluşturduk
        for (int i = 0; i < 10; i++) {
        ...
        for (int i = 0; i < 10; i++) {
            int randomSayi= new Random().nextInt(100);//Random clas methodlarıda kullanılabilir yukarısyal aynı
            //ikisi de 100 e kadar rastgele sayı seçtiriyoruz bound 100 burayla
            System.out.println("intSayi = " + randomSayi);
